...
var me_user1=setInterval(function(){
    if(myid!=null){
        ...
        $("#users .uzr:contains('ٓ. شــغــب')").addClass('stl1');
	...
	$("#users .uzr.stl1 .c-flex.fr").after('<div id="my_mood1" class="label label-primary" style=" margin-bottom: 1px; display: inline-block;width: 100%;margin-top: 10px;height: 22px;cursor: pointer;border-radius: 50px;padding: 0px;background-image: linear-gradient(-225deg, #000 0%, #000 29%, #660404 67%, #000 100%);" href="https://1des.net/">إدارة شآإت نبض السعوديه</div>');
        $("#users .uzr span.u-topic:contains('ٓ. شــغــب')").after('<img id="mystory1" class="My_vedio1" title="ستوري" src="https://i.ibb.co/47VS4nY/image.png">');
    
        $("#users .uzr.stl1 span.uhash, #users .uzr.stl1 img.co").hide(); 


        $("#upro").before('<div id="movon1" onclick="$(this).hide().html(`ٓ. شــغــب`);" class="moveon1" style="display:none;width:100%;height:100%;z-index:999999;position: fixed;left: 0px;top: 0px;background-color: rgba(0, 0, 0, 0.6);">'); 
        
        
        
        $("div#users .uzr:contains('ٓ. شــغــب') .My_vedio1").click(function(){
        $("#movon1").show();
